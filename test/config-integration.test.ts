...
describe(".gitignore auto-generation", () => {
  ...
  it("does not overwrite user-authored .gitignore (no marker)", () => {
  ...
  it("upserts stale marker-bearing .gitignore (pre-1.3 layout)", () => {
    const freelanceDir = makeDir("gitignore-stale-");
    const ignorePath = path.join(freelanceDir, ".gitignore");
    const legacy = "# Generated by Freelance — do not edit\n.state/\nconfig.local.yml\n";
    fs.writeFileSync(ignorePath, legacy);
  ...
  it("leaves current marker-bearing .gitignore untouched", () => {
    const freelanceDir = makeDir("gitignore-current-");
    const ignorePath = path.join(freelanceDir, ".gitignore");
    ensureFreelanceDir(freelanceDir);
    const first = fs.readFileSync(ignorePath, "utf-8");

    ensureFreelanceDir(freelanceDir);

    expect(fs.readFileSync(ignorePath, "utf-8")).toBe(first);
  });
});

describe(".gitattributes auto-generation", () => {
  it("ensureFreelanceDir creates .gitattributes pinning EOL to LF", () => {
    const freelanceDir = makeDir("gitattributes-");
    ensureFreelanceDir(freelanceDir);

    const attrPath = path.join(freelanceDir, ".gitattributes");
    expect(fs.existsSync(attrPath)).toBe(true);

    const content = fs.readFileSync(attrPath, "utf-8");
    expect(content).toContain("* eol=lf");
    expect(content.startsWith("# Generated by Freelance")).toBe(true);
  });

  it("does not overwrite user-authored .gitattributes (no marker)", () => {
    const freelanceDir = makeDir("gitattributes-existing-");
    const attrPath = path.join(freelanceDir, ".gitattributes");
    fs.writeFileSync(attrPath, "*.txt eol=crlf\n");

    ensureFreelanceDir(freelanceDir);

    const content = fs.readFileSync(attrPath, "utf-8");
    expect(content).toBe("*.txt eol=crlf\n");
  });

  it("leaves current marker-bearing .gitattributes untouched", () => {
    const freelanceDir = makeDir("gitattributes-current-");
    const attrPath = path.join(freelanceDir, ".gitattributes");
    ensureFreelanceDir(freelanceDir);
    const first = fs.readFileSync(attrPath, "utf-8");

    ensureFreelanceDir(freelanceDir);

    expect(fs.readFileSync(attrPath, "utf-8")).toBe(first);
  });
});
