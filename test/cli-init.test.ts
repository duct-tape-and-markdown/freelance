...
describe("CLI init", () => {
  ...
  it("appends CLAUDE.md with Freelance instructions", async () => {
    await init(defaults());
    const claudeMd = path.join(workDir, "CLAUDE.md");
    expect(fs.existsSync(claudeMd)).toBe(true);
    const content = fs.readFileSync(claudeMd, "utf-8");
    expect(content).toContain("Freelance");
    expect(content).toContain("freelance status");
  });
  ...
  it("appends to existing CLAUDE.md without duplicating", async () => {
    ...
    const matches = content2.match(/## Freelance/g);
  ...
  it("throws on invalid JSON in existing settings.json", async () => {
    const settingsDir = path.join(workDir, ".claude");
    fs.mkdirSync(settingsDir, { recursive: true });
    fs.writeFileSync(path.join(settingsDir, "settings.json"), "{ broken json !!!");
    await expect(init(defaults({ hooks: true }))).rejects.toThrow(/invalid JSON/);
  });
  ...
  it("CLAUDE.md skipped when existing content contains 'Freelance'", async () => {
    ...
    expect(content).not.toContain("freelance status");
  });
  ...
  it("writes hooks when --hooks is passed", async () => {
    ...
    expect(settings.hooks.UserPromptSubmit[0].hooks[0].command).toContain("freelance start");
  });
