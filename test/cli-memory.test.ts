...
describe("memoryEmit shape validation", () => {
  it("rejects non-array top-level with INVALID_EMIT_SHAPE", () => {
    ...
    expect(() => runCliHandler(store, () => memoryEmit(store, file))).toThrow("process.exit");
...
describe("memoryPrune refusal paths", () => {
  it("throws MISSING_KEEP when --keep is empty", () => {
    expect(() => runCliHandler(store, () => memoryPrune(store, { keep: [] }))).toThrow(
      "process.exit",
    );
    expect(stdoutJson().error?.code).toBe("MISSING_KEEP");
  });

  it("throws MISSING_KEEP when opts.keep is undefined", () => {
    expect(() => runCliHandler(store, () => memoryPrune(store, {}))).toThrow("process.exit");
    expect(stdoutJson().error?.code).toBe("MISSING_KEEP");
  });
});
...
describe("runCliHandler", () => {
  ...
  it("closes the disposable before process.exit on the error path", () => {
  ...
  it("closes the disposable on the success path too", () => {
    const closeSpy = vi.spyOn(store, "close");
    runCliHandler(store, () => {
      /* no-op success */
    });
