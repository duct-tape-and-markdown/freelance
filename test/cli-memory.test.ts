...
import { memoryEmit, memoryPrune } from "../src/cli/memory.js";
import { runCliHandler } from "../src/cli/output.js";
...
describe("memoryEmit shape validation", () => {
  it("rejects non-array top-level with INVALID_EMIT_SHAPE", async () => {
    const file = writeEmitFile(JSON.stringify({ content: "x" }));
    await expect(runCliHandler(store, () => memoryEmit(store, file))).rejects.toThrow(
      "process.exit",
    );
  ...
  it("rejects null sources with INVALID_EMIT_SHAPE (not a runtime TypeError)", async () => {
  ...
  it("rejects string-as-entities with INVALID_EMIT_SHAPE", async () => {
  ...
  it("rejects missing content with INVALID_EMIT_SHAPE", async () => {
  ...
  it("rejects empty sources array (sources: min 1 invariant)", async () => {
  ...
  it("rejects empty entities array (entities: 1..4 invariant)", async () => {
  ...
  it("rejects >4 entities (entities: 1..4 invariant)", async () => {
    const file = writeEmitFile(
  ...
  it("still rejects non-JSON with INVALID_EMIT_JSON (syntax tier unchanged)", async () => {
...
describe("memoryPrune refusal paths", () => {
  it("throws MISSING_KEEP when --keep is empty", async () => {
    await expect(runCliHandler(store, () => memoryPrune(store, { keep: [] }))).rejects.toThrow(
      "process.exit",
    );
    expect(stdoutJson().error?.code).toBe("MISSING_KEEP");
  });

  it("throws MISSING_KEEP when opts.keep is undefined", async () => {
    await expect(runCliHandler(store, () => memoryPrune(store, {}))).rejects.toThrow(
      "process.exit",
    );
    expect(stdoutJson().error?.code).toBe("MISSING_KEEP");
  });
...
describe("runCliHandler", () => {
  // `process.exit` inside `handleError` doesn't unwind `finally`, so
  // any outer `try { ... } finally { store.close() }` wouldn't run on
  // error paths. These tests lock in that the wrapper closes the
  // disposable *before* handleError fires, on both paths.
  it("closes the disposable before process.exit on the error path", async () => {
  ...
  it("closes the disposable on the success path too", async () => {
    const closeSpy = vi.spyOn(store, "close");
    await runCliHandler(store, () => {
      /* no-op success */
    });
    expect(closeSpy).toHaveBeenCalled();
  });
