...
### CI must not depend on unpinned npx tools

CI build-surface gates run from pinned `devDependencies`, never `npx -y <tool>@latest`. The `arethetypeswrong` step formerly ran `npx -y @arethetypeswrong/cli --pack .`, which re-resolves to the latest published attw on every run. When attw (≤0.18.2) is paired with fflate 0.8.3 on Node 22/24, its tarball extraction crashes for *every* package: `extractTarball` keeps only the last fflate `Gunzip` callback chunk, and fflate emits a trailing **empty** final chunk on this Node, so `untar` receives an empty buffer and dies with `Cannot read properties of undefined (reading 'filename')`. Because the spec was unpinned, this upstream regression turned every CI run red — main and all open PRs — with no project change to blame and nothing in the lockfile to bisect.

The fix pins `@arethetypeswrong/core` + `@andrewbranch/untar.js` as exact devDependencies and runs the gate via `scripts/attw-check.mjs`, which decompresses with node's `zlib.gunzipSync` (reliable, single-shot), rebuilds the `Package` file map, and runs the same `checkPackage` — mirroring the old `--ignore-rules cjs-resolves-to-esm no-resolution`. This is the same lesson as the pre-1.4 `.mcp.json` exact-pin (CHANGELOG [1.3.3]): unpinned npx specs make the build non-reproducible and hostage to upstream's release cadence.

**What would break if reversed:** any return to `npx -y <tool>` re-couples a required check to whatever that tool publishes next, with no lockfile entry recording the version that was actually exercised. A green local run no longer predicts CI. New build-surface checks belong in `devDependencies` + a `scripts/` runner, not an unpinned npx invocation.
