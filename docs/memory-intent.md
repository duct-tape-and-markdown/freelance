...
- **Unbounded accumulation is handled by explicit prune.** Over a long project with many branch switches, source rows accumulate. `freelance memory prune --keep <ref>` is the user-initiated, scope-bounded cleanup path: a row survives iff its `content_hash` matches the file on disk or at the tip of any preserve ref. Never a side effect of writes. See README §"Pruning memory" and `src/memory/prune.ts`.

Hash-as-frame is the bi-temporal model in its complete form, not an informal precursor to explicit temporal columns. `(proposition_id, file_path, content_hash)` is the claim × source-version coordinate; the read-time staleness join is the frame selector. Adding explicit `valid_from` / `valid_to` columns would duplicate what the hash already carries and introduce a second axis whose writers are either emit-time heuristics (anti-pattern) or user-initiated actions already served by `memory prune`. Inheritance across versions is automatic: content-hash dedup collapses identical claims to one `proposition_id` shared across source rows, so an unchanged claim survives a recompile as a new source row on the same proposition. See the close reasoning on #54 for the full case.
