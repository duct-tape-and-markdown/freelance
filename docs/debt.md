...
- `src/graph-validation.ts:~235` + `src/cli/validate.ts:~233` — regex-metachar escape `str.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")` is duplicated (2 callers in different layers). Factor into `src/util/regex.ts` if a third caller appears.
