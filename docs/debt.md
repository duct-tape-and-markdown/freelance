...
- `src/memory/store.ts:418`, `src/memory/store.ts:560`, `src/memory/enrichment.ts:22` — three sites construct a `NOT EXISTS (SELECT 1 FROM ${STALE_PROP_IDS_TABLE} _s WHERE _s.proposition_id = <alias>.proposition_id)` fragment with different join aliases. A `notStaleExists(alias: string): string` emitter in `staleness.ts` would put the table name + filter shape in one place. Skipped on the #154+#152+#167 PR — out of scope for the staleness-API split.
