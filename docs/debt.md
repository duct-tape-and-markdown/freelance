...
- `src/cli/program.ts` traversal verbs (status/start/advance/context-set/meta-set/inspect/reset) — same `process.exit` bypasses `finally { runtime.close() }` bug #153 fixed for memory verbs. Traversal handlers in `src/cli/traversals.ts` call `handleError` after catching, so the outer `finally` in program.ts never runs on error. Generalize `runMemoryHandler` into a disposable-wrapper (or give Runtime the memory-prune treatment) and thread across both verb groups.
- `src/cli/program.ts:~440` memory prune outer `try { memoryPrune(store) } finally { store.close() }` — prune now closes internally before every `process.exit` / in its catch, so the outer `finally` is a harmless-but-redundant belt-and-suspenders. Delete once the traversal cleanup lands and the whole memory surface is wrapper-based.
