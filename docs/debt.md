...
- `src/hook-resolution.ts:125,161` + `src/engine/hooks.ts:244` — `[...BUILTIN_HOOKS.keys()].join(", ")` repeated across three error sites for the "registered built-ins: [...]" hint. Each is on the error branch (graph-load failure or hook-runner miss), not a hot path; a `formatBuiltinHookList()` helper next to `BUILTIN_HOOKS` would dedup. Out of scope on #219 (which only deleted the `BUILTIN_HOOK_NAMES` Set projection).
